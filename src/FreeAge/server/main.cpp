...
void HandleStartGame(const QByteArray& /*msg*/, PlayerInMatch* player, const std::vector<std::shared_ptr<PlayerInMatch>>& playersInMatch) {
  if (player->isHost) {
  ...
  QByteArray msg = CreateStartGameNotifyMessage();
  for (const auto& otherPlayer : playersInMatch) {
    if (otherPlayer->state == PlayerInMatch::State::Joined) {
      otherPlayer->socket->write(msg);
    }
