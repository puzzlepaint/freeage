...
public:
    ...
    /// Variant of offline insertion which outputs the mapping between input and output indices.
    /// For each item in dst, the corresponding item in dstOriginalIndices gives its original index in the rects vector.
    void Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, std::vector<int> &dstOriginalIndices, FreeRectChoiceHeuristic method);
