...
void MaxRectsBinPack::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, std::vector<int> &dstOriginalIndices, FreeRectChoiceHeuristic method)
{
    std::vector<int> originalIndices(rects.size());
    ...
    while(rects.size() > 0)
    {
        ...
        PlaceRect(bestNode);
        dst.push_back(bestNode);
        dstOriginalIndices.push_back(originalIndices[bestRectIndex]);
        rects.erase(rects.begin() + bestRectIndex);
        originalIndices.erase(originalIndices.begin() + bestRectIndex);
    }
}
